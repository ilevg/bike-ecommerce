...
const Blog = () => {
    ...
    const displayPosts = showMore ? posts.slice().map(formatPostData) : posts.slice(0, 6).map(formatPostData);
    ...
    return (
        <>
            ...
            <div className={classNames(styles.blog, 'container')}>
                ...
                {!showMore && (
                    <button className={styles.blogBtn} type='button' onClick={handleShowMore}>Show more...</button>
                )}
            </div>
        </>
    )
