  ...
  return (
    <div>
      ...
      <div className={classNames('container', styles.post)}>
        <h2 className={styles.bannerSubtitle}>{data.length > 0 && data[0].content}</h2>
        <p className={styles.bannerDesc}>{data.length > 0 && data[1].content[0]}</p>
        <img className={styles.bannerImg} src={data.length > 0 ? data[1].content[1].src : ''} alt={data.length > 0 ? data[1].content[1].alt : ''} />

        {data.length > 0 && data.slice(2).map((item, index) => (
          <div key={index}>
            {item.type === 'heading' && <h3 className={styles.bannerSubtitle}>{item.content}</h3>}
            {item.type === 'description'
              ?
              (
                <div className={index % 3 ? styles.postItem : styles.postItemReverse}>
                  {<p className={styles.desc}>{item.content[0]}</p>}
                  {<img className={styles.imgDesc} src={item.content[1].src} alt={item.content[1].alt} />}
                </div>
              )
              : null}
          </div>
