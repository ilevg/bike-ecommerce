export function parserHtmlContent(html) {
    ...
    const headings = Array.from(doc.querySelectorAll('h3')).map(heading => heading.textContent.trim());
    const descriptions = Array.from(doc.querySelectorAll('p.has-small-font-size')).map(p => p.textContent.trim());
